...
def predict(face_aligned, svc, threshold=0.7):
    ...
    try:
        x_face_locations = face_recognition.face_locations(face_aligned, model="cnn")
...
def data_visualizer(embedded, targets,):

    embeddedx = TSNE(n_components=2).fit_transform(embedded)

    for i, t in enumerate(set(targets)):
        idx = targets == t
        plt.scatter(embeddedx[idx, 0], embeddedx[idx, 1], label=t)
...
def db_attendance_updater(present):
...
def db_update_attendance_out(present):
    today = datetime.date.today()
    time = datetime.datetime.now()
    for person in present:
        user = User.objects.get(username=person)
...
def validity_timing_check(times_all):
...
def student_hours(present_qs, time_qs, admin=True):
    ...
    for obj in qs:
        ...
        (check, break_hourss) = validity_timing_check(times_all)
        if check:
        ...
        obj.hours = htominsconverter(obj.hours)
        obj.break_hours = htominsconverter(obj.break_hours)
...
# used
def student_given_date_hours(present_qs, time_qs):
    register_matplotlib_converters()
    df_hours = []
    df_break_hours = []
    df_username = []
...
def weekly_student_count():
    ...
    while(cnt < 5):
        ...
        if(str_dates.count(date)) > 0:
            ...
            stu_cnt_all.append(stu_count[idx])
        else:
            stu_cnt_all.append(0)

    df = pd.DataFrame()
    df["date"] = str_dates_all
    df["Number of students"] = stu_cnt_all
...
def last_week_students():
    ...
    df = pd.DataFrame()
    df["date"] = str_dates_all
    df["stu_count"] = stu_cnt_all

    sns.lineplot(data=df, x='date', y='stu_count')
    plt.savefig(
...
def mark_your_attendance(request):
    ...
    vs.stop()
    cv2.destroyAllWindows()
    db_attendance_updater(present)
    return redirect('home')

#Mark your attendance-out
#Input: User will scan his/her face using the external web camera.
#Output: system will identify the user uniquely and will mark his/her out-time to the database
#The same success message will be transmitted to the user.
...
def mark_your_attendance_out(request):
    ...
    vs.stop()
    cv2.destroyAllWindows()
    db_update_attendance_out(present)
    return redirect('home')

#Train the system
#Input: user selection
#Output: system will process all the available records of the students and will 
#generate necessary system data to identify each student uniquely.
...
#student till the date. i.e., how many students are present today out of total
#students etc. can be monitored.
#Input: user selection
#Output: Attendance record of each student including how many students are
#present today out of total along with the availability graph.

@login_required
def view_attendance_home(request):
    total_num_of_stu = students_sum()
    stu_present_today = students_today()
    weekly_student_count()
    last_week_students()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if(len(time_qs) > 0 or len(present_qs) > 0):
                qs = student_given_date_hours(present_qs, time_qs)
...
@login_required
def view_student_attendance_obuda(request):
    if request.user.username != 'admin':
        return redirect('not-authorised')
    time_qs = None
    present_qs = None
        ...
        if form.is_valid():
            ...
            if usernamefound(username):
                ...
                if date_to < date_from:
                    ...
                    return redirect('view_student_attendance_obuda')
                else:
                    ...
                    if (len(time_qs) > 0 or len(present_qs) > 0):
                        qs = student_hours(present_qs, time_qs, admin=True)
                        return render(request, 'recognition/view_student_attendance_obuda.html', {'form': form, 'qs': qs})
                    else:
...
#Description: student may often need to see his / her attendance record throughout
#the month or year. Using this feature one can see his / her attendance record till the
#date.
#Input: User selection
#Output: Statistical analytics of the particular student who is currently logged into
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if date_to < date_from:
                ...
                if (len(time_qs) > 0 or len(present_qs) > 0):
                    qs = student_hours(present_qs, time_qs, admin=False)
